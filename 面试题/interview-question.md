...
树形表格虚拟渲染需要一个通过id 和 parentId 来确定依赖关系的数组

树形表格的虚拟渲染的主要难点是主要是如何将一个不规则的、无序的数组制作成有序的数组。

```
// 把一个散的数组做成一个有顺序的数组
// 把所有的 parent 拎起来，list 是一个对象，遍历散数组，以 parentId 为 key，将所有的子项挂到 parent 上面。
// 此时 list 对象的每一项是个数组，遍历这个数组，给数组的每一项挂上 children，这个 children 就在 list 这个 map 里面去取。
// 已经挂上了 children，由于引用类型，所以只需要取 list[0] 这一项，就可以拿到一个从根开始的一颗树
// 递归这棵树，就可以生成一个根据 id 和 parentId 排好序一个数组。
```

有了这个有序的数组，就可以根据滚动位置 scrollLeft、scrollTop 、表格高度、每一行的高度来计算需要显示的数据。
